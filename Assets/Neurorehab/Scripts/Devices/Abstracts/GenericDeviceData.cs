...
namespace Neurorehab.Scripts.Devices.Abstracts
{
    ...
    public class GenericDeviceData
    {
        ...
        {
            var hasValues = HasRotations() || HasPositions() || HasFloats() || HasBooleans() || HasSamples();
        ...
        {
            SetFloat(values.InformationLabel, float.Parse(values.Values[0]));
        }
        ...
        protected virtual void ProcessSample(StringValues values)
        {
            ...
            foreach (var val in values.Values)
                sampleValues.Add(float.Parse(val));
        ...
        public void StopThreads()
        {
            ...
            Debug.Log(CleanDictionariesThread.ThreadState);
        }
    }
}